...
function renderWidgetContent(type: WidgetType, title: string) {
  switch (type) {
    ...
    default:
      return <div className="flex h-full items-center justify-center text-gray-500">{title}</div>;
  }
}
...
export function DashboardPage() {
  ...
  const moveWidget = (dragIndex: number, hoverIndex: number) => {
    setWidgets((prev) => {
      ...
      const [draggedWidget] = next.splice(dragIndex, 1);
      next.splice(hoverIndex, 0, draggedWidget);
      return next.map((widget, index) => ({ ...widget, order: index + 1 }));
    });
  };
        ...
        {[...widgets]
          ...
          .map((widget, index) => (
            <DraggableWidget
              ...
              onRemove={() => setWidgets((prev) => prev.filter((item) => item.id !== widget.id))}
              onResize={(colSpan, rowSpan) => setWidgets((prev) => prev.map((item) => (item.id === widget.id ? { ...item, colSpan, rowSpan } : item)))}
            >
