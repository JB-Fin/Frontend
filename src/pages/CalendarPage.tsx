...
const emptyForm = {
  title: '', date: TODAY, endDate: TODAY,
  timeStart: '', timeEnd: '', location: '',
  department: '', category: '세미나', memo: '',
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setForm({ ...emptyForm, date: selectedDate, endDate: selectedDate }); setShowModal(true) }}
        ...
        <div className="col-span-2 rounded-lg border border-white/60 bg-white/85 p-6 shadow-lg backdrop-blur-xl">
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1))} className="rounded-lg p-2 transition-colors hover:bg-gray-100/80">
                <ChevronLeft className="h-5 w-5 text-gray-600" />
              </button>
              <button onClick={() => { setCurrentDate(new Date(2026, 5, 8)); setSelectedDate(TODAY) }} className="rounded-lg bg-blue-50/80 px-4 py-2 text-sm font-medium text-blue-600 transition-colors hover:bg-blue-100/80">
                오늘
              </button>
              <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1))} className="rounded-lg p-2 transition-colors hover:bg-gray-100/80">
          ...
          <div className="grid grid-cols-7 gap-2">
            {dayNames.map((day, index) => (
              <div key={day} className={`py-2 text-center text-sm font-medium ${index === 0 ? 'text-red-600' : index === 6 ? 'text-blue-600' : 'text-gray-600'}`}>
                {day}
              </div>
            ))}
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={form.title} onChange={e => handleFormChange('title', e.target.value)} placeholder="일정 제목을 입력하세요" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(cat => {
                    ...
                    return (
                      <button key={cat} onClick={() => handleFormChange('category', cat)}
                        className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium transition-all ${
                          active ? `border-transparent text-white shadow-sm ${CATEGORY_COLOR_MAP[cat]}` : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300 hover:bg-gray-100'
                        }`}
                      >
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="date" value={form.date} onChange={e => handleFormChange('date', e.target.value)} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                  <span className="flex-shrink-0 text-sm text-gray-400">~</span>
                  <input type="date" value={form.endDate} min={form.date} onChange={e => handleFormChange('endDate', e.target.value)} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" value={form.timeStart} onChange={e => handleFormChange('timeStart', e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">종료 시간</label>
                  <input type="time" value={form.timeEnd} onChange={e => handleFormChange('timeEnd', e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                </div>
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">장소</label>
                <input type="text" value={form.location} onChange={e => handleFormChange('location', e.target.value)} placeholder="장소를 입력하세요" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">관할 부서</label>
                <input type="text" value={form.department} onChange={e => handleFormChange('department', e.target.value)} placeholder="예) 준법감시팀, 리스크관리팀" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">메모</label>
                <textarea value={form.memo} onChange={e => handleFormChange('memo', e.target.value)} placeholder="추가 메모를 입력하세요" rows={3} className="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
            </div>

            <div className="mt-6 flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 rounded-lg border border-gray-300 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">취소</button>
              <button onClick={handleAddEvent} disabled={!form.title || !form.date} className="flex-1 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 py-2.5 text-sm font-medium text-white hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50">추가</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium text-white ${event.color}`}>{event.category}</span>
          </div>
          ...
          {!compact && event.memo && (
            <p className="mt-2 rounded-md bg-gray-50 px-2 py-1.5 text-xs leading-relaxed text-gray-500">{event.memo}</p>
          )}
        </div>
      </div>
    </div>
